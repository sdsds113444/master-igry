...
-- ---------------------------------------------------------------------------
-- 7. СИДЫ: 7 игр сезона. Кейсы грузятся ОТДЕЛЬНО из supabase/cases.sql
--    (единственный источник кейсов — не дублируем их здесь).
-- ---------------------------------------------------------------------------
...
-- ============================================================================
--  8. КОМАНДЫ И АДМИН-КОД — НЕ храним в репозитории!
--  ----------------------------------------------------------------------------
--  Коды входа — это секреты. НЕ сидим их в этом файле (он лежит в git и попадёт
--  в чужие руки). Заводим коды отдельно, ВЫСОКОЭНТРОПИЙНЫМИ, и раздаём вручную.
--
--  Боевые команды (пример генерации; правь названия/тренеров после):
--    insert into public.teams (code, name, site, mentor, hue)
--    select 'KOYA-' || lpad(n::text,2,'0') || '-' || encode(gen_random_bytes(4),'hex'),
--           'Команда ' || n, null, null, (n*37) % 360
--    from generate_series(1,30) as n;
--
--  Админ-код (одноразово, длинный, НЕ коммитить значение никуда):
--    insert into public.admin_codes (code, label)
--    values ('ADMIN-' || encode(gen_random_bytes(9),'hex'), 'Организатор');
--    -- затем прочитай значение из Table Editor и передай организатору лично.
--
--  Демо БЕЗ базы (визуал/логика) — работает на моках (src/data/mock.ts),
--  реальные секреты для этого не нужны.
-- ============================================================================
